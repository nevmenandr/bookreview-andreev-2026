#!/usr/bin/env python3
"""
Воспроизведение подсчетов Андреева из Главы 2 на корпусе new-corpus.
Сравнение результатов с данными из монографии.
Результаты сохраняются в файл result_chapter2_newcorpus.txt
"""
...
RESULTS_FILE = Path("/andreev_chapter2_newcorpus_results.json")
OUTPUT_FILE = Path("/result_chapter2_newcorpus.txt")
...
# Сопоставление сборников из new-corpus с названиями у Андреева
# В new-corpus тексты разбиты на early и late, но для главы 2 нам нужны
# конкретные сборники. Используем эвристики для определения сборника
# по имени файла или содержимому.
...
def determine_collection_from_text(text):
    """
    Определяет сборник по тексту на основе метаданных в начале файла.
    """
    lines = text.split('\n')
    for line in lines[:20]:  # Проверяем первые 20 строк
        if line.startswith('#') and 'Год' in line:
            # Извлекаем год
            year_match = re.search(r'(\d{4})', line)
            if year_match:
                year = int(year_match.group(1))
                if year < 1926:
                    return "Горний путь"  # Ранние сборники
                elif 1926 <= year <= 1928:
                    return "Гроздь"
                elif 1929 <= year <= 1931:
                    return "Возвращение Чорба"
                elif 1932 <= year <= 1951:
                    return "Стихотворения 1929-1951"
        elif line.startswith('#') and 'сборник' in line.lower():
            # Пробуем найти название сборника
            for collection in ["Гроздь", "Горний путь", "Возвращение Чорба", "Стихотворения 1929-1951"]:
                if collection.lower() in line.lower():
                    return collection
    
    # Если не удалось определить по метаданным, пробуем по году в имени файла
    return None
...
def compare_results(our_results, andreev_results):
    ...
    for collection in andreev_results.keys():
        ...
        # Если наши данные пустые, пропускаем
        if not our:
            continue
...
def write_output(comparison, results, output_file):
    ...
    with open(output_file, 'w', encoding='utf-8') as f:
        ...
        if summary['accuracy'] == "Отличная":
            ...
            f.write("    - разным составом текстов в корпусе\n")
            ...
            f.write("    - уточнить методику POS-тэггинга\n")
...
def main():
    ...
    # Группируем тексты по сборникам
    collection_poems = defaultdict(list)
    
    # Определяем сборник для каждого текста
    for period in ["early", "late"]:
        for poem in poems_by_period[period]:
            collection = determine_collection_from_text(poem["text"])
            if collection:
                collection_poems[collection].append(poem)
            else:
                # Если не удалось определить, используем период
                if period == "early":
                    collection_poems["Горний путь (early)"].append(poem)
                else:
                    collection_poems["Стихотворения 1929-1951 (late)"].append(poem)
    
    print(f"\n📚 Распределение по сборникам:")
    for collection, poems in collection_poems.items():
        print(f"  {collection}: {len(poems)} текстов")
        for p in poems[:3]:
            print(f"    - {p['code']}")
        if len(poems) > 3:
            print(f"    ... и еще {len(poems) - 3}")
    
    if not collection_poems:
        print("\n❌ Не удалось определить сборники")
        return
    ...
    # Оставляем только те сборники, которые есть у Андреева
    filtered_results = {}
    for collection, data in results.items():
        # Пытаемся сопоставить с названиями из ANDREEV_DATA
        matched = False
        for andreev_collection in ANDREEV_DATA.keys():
            if andreev_collection in collection or collection in andreev_collection:
                filtered_results[andreev_collection] = data
                matched = True
                break
        if not matched:
            # Пробуем по году
            for andreev_collection in ANDREEV_DATA.keys():
                if "early" in collection and "Горний" in andreev_collection:
                    filtered_results[andreev_collection] = data
                    matched = True
                    break
                elif "late" in collection and "Стихотворения" in andreev_collection:
                    filtered_results[andreev_collection] = data
                    matched = True
                    break
    
    # Сравнение
    comparison = compare_results(filtered_results, ANDREEV_DATA)
    ...
    # Записываем в файл
    write_output(comparison, filtered_results, OUTPUT_FILE)
